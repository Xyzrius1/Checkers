...
public class CheckersGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (isValidMoveInput(moveInput)) {
                ...
                int fromRow = Integer.parseInt(move[0].charAt(1) + "") - 1; // Convert to 0-index
                int fromCol = move[0].charAt(0) - 'A'; // Convert letter to index
                int toRow = Integer.parseInt(move[1].charAt(1) + "") - 1; // Convert to 0-index
    ...
    private static void initializeBoard() {
        ...
        // Set up black pieces (b)
        for (int row = 0; row < 3; row++) {
            for (int col = (row % 2); col < BOARD_SIZE; col += 2) {
                board[row][col] = 'b'; // Player 1 pieces
            }
        }
        // Set up white pieces (w)
        for (int row = 5; row < BOARD_SIZE; row++) {
            for (int col = (row % 2); col < BOARD_SIZE; col += 2) {
                board[row][col] = 'w'; // Player 2 pieces
            }
        }
    }
        ...
        if (isPlayer1) {
            if (currentPiece == 'b') {
                ...
                if (rowDiff == 2 && Math.abs(colDiff) == 2) {
                    ...
                    return board[capturedRow][capturedCol] == 'w'; // Check for a white piece to capture
                }
            }
        } else {
            if (currentPiece == 'w') {
                ...
                if (rowDiff == -2 && Math.abs(colDiff) == 2) {
                    int capturedRow = fromRow + (rowDiff / 2);
                    int capturedCol = fromCol + (colDiff / 2);
                    return board[capturedRow][capturedCol] == 'b'; // Check for a black piece to capture
                }
            }
        }
